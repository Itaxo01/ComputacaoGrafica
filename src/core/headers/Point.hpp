#ifndef POINT_H
#define POINT_H
#define float_64 long double
#define EPS 1e-9 // This is our threshold for something like x == y on floats, there could be a tiny error that the EPS will handle
...
class Point{
    float_64 x, y;
    Point(float_64 x = 0, float_64 y = 0): x(x), y(y){}

    friend Point operator+(const Point &p, const Point &q) {return Point(p.x + q.x, p.y + q.y);}
    friend Point operator-(const Point &p, const Point &q) {return Point(p.x - q.x, p.y - q.y);}
    friend Point operator*(const Point &p, const float_64 k) {return Point(p.x * k, p.y * k);}
    friend Point operator/(const Point &p, const float_64 k) {return Point(p.x / k, p.y / k);}
    Point operator+=(const Point &q) {return Point(this->x+q.x, this->y+q.y);}
    Point operator-=(const Point &q) {return Point(this->x-q.x, this->y-q.y);}
    Point operator*=(const float_64 k) {return Point(this->x*k, this->y*k);}
    Point operator/=(const float_64 k) {return Point(this->x/k, this->y/k);}
    ...
    friend float_64 dot(const Point &p, const Point&q) {
        return p.x * q.x + p.y * q.y;   
    }
    /**Distance between points without square root */
    friend float_64 dist2(const Point&p, const Point&q) {return dot(p-q, p-q);}
    /**Distance between points with square root */
    friend float_64 dist(const Point&p, const Point&q) {return sqrtl(dist2(p, q));}
    /* Cross product, useful to find the orientation (This is a 3D operation, but it's simplified with Z = 0)*/
    friend float_64 cross(const Point &p, const Point &q) {return p.x * q.y - p.y * q.x;}
    /**Projection of one vector in to another*/
    friend float_64 proj(const Point&p, const Point&q) {return dot(p, q) / (dist(p, q));}
