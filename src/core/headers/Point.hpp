...
class Point{
    public:
        float_32 x, y;
        Point(float_32 x = 0, float_32 y = 0): x(x), y(y){}

        friend Point operator+(const Point &p, const Point &q) {return Point(p.x + q.x, p.y + q.y);}
        friend Point operator-(const Point &p, const Point &q) {return Point(p.x - q.x, p.y - q.y);}
        friend Point operator*(const Point &p, const float_32 k) {return Point(p.x * k, p.y * k);}
        friend Point operator/(const Point &p, const float_32 k) {return Point(p.x / k, p.y / k);}
        Point operator+=(const Point &q) {return Point(this->x+q.x, this->y+q.y);}
        Point operator-=(const Point &q) {return Point(this->x-q.x, this->y-q.y);}
        Point operator*=(const float_32 k) {return Point(this->x*k, this->y*k);}
        Point operator/=(const float_32 k) {return Point(this->x/k, this->y/k);}
        ...
        friend float_32 dot(const Point &p, const Point&q) {
            return p.x * q.x + p.y * q.y;   
        }
        /**Distance between points without square root */
        friend float_32 dist2(const Point&p, const Point&q) {return dot(p-q, p-q);}
        /**Distance between points with square root */
        friend float_32 dist(const Point&p, const Point&q) {return sqrtl(dist2(p, q));}
        /* Cross product, useful to find the orientation (This is a 3D operation, but it's simplified with Z = 0)*/
        friend float_32 cross(const Point &p, const Point &q) {return p.x * q.y - p.y * q.x;}
        /**Projection of one vector in to another*/
        friend float_32 proj(const Point&p, const Point&q) {return dot(p, q) / (dist(p, q));}
